...
export async function register(name, email, password){
  ...
  await set(ref(db, `users/${cred.user.uid}`), { uid:cred.user.uid, name, email, role:'member', title:'Project Member', avatar:'', createdAt:Date.now(), lastLogin:Date.now() });
  return cred;
}
...
export async function touchUser(user){
  await update(ref(db, `users/${user.uid}`), { lastLogin:Date.now(), name:user.displayName || user.email });
}
